...
每个题目需要创建独立的文件夹，命名格式：`四位题号_题目英文名（下划线分隔）`

**文件夹命名示例**：
- 题号 1 → `0001_two_sum`
- 题号 139 → `0139_word_break`
- 题号 300 → `0300_longest_increasing_subsequence`

题号不足四位时**左侧补零至四位**，与仓库现有目录风格保持一致。
...
每次生成新题目后，**必须**同步更新仓库根目录的 [`INDEX.md`](../../INDEX.md) 索引文件。

**表格行格式**（与现有条目保持一致）：

```markdown
| ⬜ | [题号] | [题目中文名](https://leetcode.cn/problems/[slug]/) | [[文件夹名]](./[文件夹名]/) |
```

**说明**：
- **状态**：新建题目用 `⬜`（未完成）；用户明确已完成时可标 `✅`
- **题号**：LeetCode 题号（数字，无前导零）
- **题目链接**：力扣中文版 URL，`slug` 与官方题目路径一致（如 `two-sum`、`3sum`）
- **目录链接**：相对路径 `./四位题号_题目英文名/`，与新建文件夹名一致（如 `./0300_longest_increasing_subsequence/`）

**归类与排序**：
- 将新题目加入对应的 `## 专题` 小节（如：哈希、双指针、普通数组等）
- 若用户未指定专题，根据题意判断；无法判断时询问用户
- 若该专题尚不存在，在文件末尾新增 `## 专题名` 及表头行
- 同一专题内按学习顺序或题号逻辑插入（例如「四数之和」放在「三数之和」之后）
- 若索引中已有该题，只更新必要字段，**不要**重复添加
