...
当用户请求生成 LeetCode 题目时，你需要按照以下规则创建三个文件。
...
当用户使用此技能时：
1. 首先从用户的输入中提取 LeetCode 题目信息（可以包括题目编号、题目中文名、题目英文名、题目描述、示例、提示等）
2. 如果用户只提供了题目名称（如 "/leetcode 121. 买卖股票的最佳时机" 或 "/leetcode Best Time to Buy and Sell Stock"），你需要通过网络搜索获取完整的题目信息
3. 按照上述规则生成三个文件
4. 确保所有文件都按照格式要求创建，并且 test.py 和 debug.py 中的测试用例正确
5. **不要实现 solution.py 中的具体代码**，只保留 `pass`
6. 告诉用户题目文件已创建，可以开始实现了
