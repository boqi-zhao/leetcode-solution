...
def test_example1():
    ...
    print(f"输入: nums = {[1, 2, 3, 4, 5, 6, 7]}, k = {k}")
...
def test_example2():
    ...
    print(f"输入: nums = [-1, -100, 3, 99], k = {k}")
    print(f"输出: nums = {nums}")
    print(f"期望: {expected}")
    print(f"结果: {'✅ 通过' if nums == expected else '❌ 失败'}")
...
def test_edge_case_k_zero():
    ...
    print(f"输入: nums = [1, 2, 3], k = {k}")
    print(f"输出: nums = {nums}")
    print(f"期望: {expected}")
    print(f"结果: {'✅ 通过' if nums == expected else '❌ 失败'}")


def test_edge_case_k_greater_than_length():
    """测试边界情况 - k 大于数组长度"""
    solution = Solution()
    nums = [1, 2, 3]
    k = 4
...
if __name__ == '__main__':
    ...
    # test_edge_case_k_zero()              # 运行边界情况 - k 为 0
    # test_edge_case_k_greater_than_length()  # 运行边界情况 - k 大于长度
    # test_custom()      # 运行自定义测试
